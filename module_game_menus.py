...
#
# [ Z01 ] - Hardcoded Menus
# [ Z02 ] - Character Generation (Removed)
# [ Z03 ] - Reports
# [ Z04 ] - Camp
# [ Z05 ] - Cattle Herds
# [ Z06 ] - Duels
...
####################################################################################################################
  ...
  # Adicionado o primeiro rascunho do texto introdutorio ao Dark Tower.
  ("start_game_0",menu_text_color(0xFF000000)|mnf_disable_all_keys,
    "Only in the All-World we find the manifestation of the nexus of worlds as a Tower.\
 There, inside this Dark Tower stretching as a monolith through the sky, you can climb its stairs and reach the top.\
 No one knows who or what stands on its last floor, but your eargeness and curiosity is enough to try to find out.\
 The tower is now the single goal in your mind.\
 You are tirelessly seeking a wizard that you believe to be the only one with\
 answer for your question: 'How do I find it?' But yet, never getting closer and alway from afar...",
    "none",
    [],
    [
     ("continue",[],"...the man in black fled across the desert, and you followed...",
     [
     (assign, "$option", -1),
     (play_sound, "snd_intro_wind_desert"),
     (start_presentation, "prsnt_mcc_character_creation"),  # Character Creation Scene by mercury19
      ]
       ),

      ("go_back",[],"Let it crumble...",
       [
         (change_screen_quit),
       ]),
    ]
...
# Editado: Character Creation Scene by mercury19
  ("start_phase_2",mnf_disable_all_keys, #start_phase_2_5
         ...
         [
          ...
          (eq, ":continue", 1),
            ...
            [
               ("continue",[], "The desert was the apotheosis of all deserts...",
                [         
...
###########################
  ...
  ( "tutorial",mnf_disable_all_keys,
    "You approach a field where gunslingers are training with weapons. You can practice here to improve your combat skills.",
    "none",
    [
      (try_begin),
...
# TEST WEAPONS HERE
        ...
        #(troop_add_item, "$g_player_troop","itm_flintlock_pistol",0),
        #(troop_add_item, "$g_player_troop","itm_sandalwood_gun",0),
        (troop_add_item, "$g_player_troop","itm_british_grenade1",0),
...
# Towns
 ...
 but no banner is flown above the gates. Zendar is proud of keeping its neutrality and providing a safe haven to anyone in need of refuge.",
    ...
    [
	  ...
	  #Ramaraunt ADD
	 ("visit_port",
       [(assign,reg0,zendar_port),
        ],
         "Go to the port.",
         [ (assign, "$g_next_menu", "mnu_zendar"),
           (jump_to_menu,"mnu_zendar_port"),
          ]),
		#Ramaraunt ADD
...
####################################################################################################################   
  ...
  #Ramaraunt ADD
  ( "zendar_port",0,
    "You are at the port of Zendar. Merchant ships are flowing steadily through the harbor gates, and you can almost hear the sound of coins changing hands. {s4}",
    "none",
    [(str_store_party_name, s2, zendar_town),
	 (assign, "$temp", -1),
	 (try_for_range, ":cur_captain", tavern_captains_begin, tavern_captains_end),
	   (troop_slot_eq, ":cur_captain", slot_troop_cur_center, "$current_town"),
	   (this_or_next|lt, "$temp", 0),
    ...
    [
      ("charter_ship", [(eq, reg5, 1), (le, "$players_captain", 0)],
      "Charter ship.",
      [ (party_get_position, pos1, zendar_port),
        (assign, "$g_player_icon_state", pis_ship),
        (party_set_flags, "p_main_party", pf_is_ship, 1),
        (map_get_water_position_around_position, pos2, pos1, 1),
        (party_set_position, "p_main_party", pos2),
